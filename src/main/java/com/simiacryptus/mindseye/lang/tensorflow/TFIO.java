...
public class TFIO {
  ...
  public static org.tensorflow.Tensor<Float> getFloatTensor(TensorList data, boolean invertRanks) {
    ...
    long[] shape = LongStream
        .concat(LongStream.of(data.length()), Arrays.stream(data.getDimensions()).mapToLong(x -> x)).toArray();
    @NotNull
  ...
  private static Object createFloatArray(long[] shape) {
    if (shape.length == 1) {
      ...
      return IntStream.range(0, (int) shape[0]).mapToObj(i -> new float[(int) shape[1]]).toArray(s -> new float[s][]);
    } else if (shape.length == 3) {
      return IntStream.range(0, (int) shape[0]).mapToObj(
          i -> IntStream.range(0, (int) shape[1]).mapToObj(j -> new float[(int) shape[2]]).toArray(s -> new float[s][]))
          .toArray(s -> new float[s][][]);
    } else if (shape.length == 4) {
      return IntStream.range(0, (int) shape[0])
          .mapToObj(i -> IntStream
              .range(0, (int) shape[1]).mapToObj(j -> IntStream.range(0, (int) shape[2])
      ...
      return IntStream.range(0, (int) shape[0])
          .mapToObj(i -> IntStream.range(0, (int) shape[1])
              .mapToObj(j -> IntStream.range(0, (int) shape[2])
                  .mapToObj(k -> IntStream.range(0, (int) shape[3]).mapToObj(l -> new float[(int) shape[4]])
                      .toArray(s -> new float[s][]))
                  .toArray(s -> new float[s][][]))
              .toArray(s -> new float[s][][][]))
          .toArray(s -> new float[s][][][][]);
    } else if (shape.length == 6) {
      return IntStream.range(0, (int) shape[0]).mapToObj(i -> IntStream.range(0, (int) shape[1])
          .mapToObj(j -> IntStream.range(0, (int) shape[2])
              .mapToObj(k -> IntStream.range(0, (int) shape[3])
                  .mapToObj(l -> IntStream.range(0, (int) shape[4]).mapToObj(m -> new float[(int) shape[5]])
                      .toArray(s -> new float[s][]))
                  .toArray(s -> new float[s][][]))
              .toArray(s -> new float[s][][][]))
  ...
  private static Object createDoubleArray(long[] shape) {
    if (shape.length == 1) {
      ...
      return IntStream.range(0, (int) shape[0]).mapToObj(i -> new double[(int) shape[1]]).toArray(s -> new double[s][]);
    } else if (shape.length == 3) {
      return IntStream.range(0, (int) shape[0]).mapToObj(i -> IntStream.range(0, (int) shape[1])
      ...
      return IntStream.range(0, (int) shape[0])
          .mapToObj(
              i -> IntStream.range(0, (int) shape[1])
                  .mapToObj(j -> IntStream.range(0, (int) shape[2]).mapToObj(k -> new double[(int) shape[3]])
  ...
  private static TensorArray getTensorArray_Float(org.tensorflow.Tensor<Float> tensor, long[] shape,
    ...
    TensorArray resultData = new TensorArray(IntStream.range(0, batches).mapToObj(i -> {
  ...
  private static Tensor getTensor_Float(org.tensorflow.Tensor<Float> tensor, long[] shape, boolean invertRanks) {
    if (0 == tensor.numElements())
      return new Tensor(Arrays.stream(shape).mapToInt(x -> (int) x).toArray());
    float[] doubles = getFloats(tensor);
