...
public abstract class MnistDemoBase {
  ...
  public void run(@Nonnull NotebookOutput log) {
    ...
    final Tensor[][] trainingData = MNIST.trainingDataStream().map(labeledObject1 -> {
      ...
      Tensor[] temp_06_0001 = new Tensor[] { labeledObject1.data, categoryTensor == null ? null : categoryTensor };
      return temp_06_0001;
    ...
    log.h1("Training");
    @Nonnull
    final RefList<Step> history = new RefArrayList<>();
    @Nonnull
    ...
    log.eval(RefUtil.wrapInterface((UncheckedSupplier<Double>) () -> {
      ...
      @Nonnull
      final SimpleLossNetwork supervisedNetwork = new SimpleLossNetwork(
          recognitionNetwork == null ? null : recognitionNetwork.addRef(), loss == null ? null : loss.addRef());
      if (null != loss)
        loss.freeRef();
      @Nonnull
      final Trainable trainable = new SampledArrayTrainable(Tensor.addRefs(trainingData),
          supervisedNetwork == null ? null : supervisedNetwork, 1000, 1000);
      IterativeTrainer temp_06_0008 = new IterativeTrainer(trainable == null ? null : trainable);
    ...
    @Nonnull
    final String modelName = "model.json";
      ...
      RefList<int[]> predicitonList = RefIntStream.range(0, predictionData.length())
          .mapToObj(RefUtil.wrapInterface((IntFunction<? extends int[]>) rowIndex -> {
            ...
            int[] temp_06_0005 = RefIntStream.range(0, 10).mapToObj(x -> x).sorted(
                ...
                }, predictionTensor == null ? null : predictionTensor.addRef()))).mapToInt(x -> x).toArray();
            if (null != predictionTensor)
              predictionTensor.freeRef();
            return temp_06_0005;
          }, predictionData == null ? null : predictionData.addRef())).collect(RefCollectors.toList());
      double temp_06_0004 = RefIntStream.range(0, predictionData.length())
          .mapToDouble(RefUtil.wrapInterface(rowIndex -> {
            return predicitonList.get(rowIndex)[0] == parse(validation.get(rowIndex).label) ? 1 : 0;
          }, validation == null ? null : validation.addRef(), predicitonList == null ? null : predicitonList.addRef()))
    ...
    log.eval(RefUtil.wrapInterface((UncheckedSupplier<TableOutput>) () -> {
      ...
      MNIST.validationDataStream().map(RefUtil.wrapInterface(
          (Function<? super LabeledObject<Tensor>, ? extends RefLinkedHashMap<CharSequence, Object>>) labeledObject -> {
            ...
            @Nullable
            final double[] predictionSignal = temp_06_0018.getData();
            ...
            @Nonnull
            final RefLinkedHashMap<CharSequence, Object> row = new RefLinkedHashMap<>();
